...
} from 'src/app/system-apps/window/animation/animations';
...
export class WindowComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
	...
	ngOnChanges(changes: SimpleChanges): void {
		...
		if (this.name == 'Window') this.name = this.processAppName;
	...
	setMaximizeAndUnMaximize(): void {
		...
		if (this.windowMaximize) {
			...
			if (windowState.pid == this.processId) {
				...
				this._runningProcessService.addEventOriginator(uid);
				this._runningProcessService.minimizeWindowNotify.next([
					windowState.width,
					windowState.height - windowTitleBarHeight,
				]);
			}
		}
	...
	removeFocusOnWindow(pid: number): void {
		const processWithWindows = this._runningProcessService
			.getProcesses()
			.filter((p) => p.getHasWindow == true && p.getProcessId != pid);

		for (let i = 0; i < processWithWindows.length; i++) {
			const process = processWithWindows[i];
			const window = this._stateManagmentService.getState(
