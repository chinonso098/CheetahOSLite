...
export async function getFileInfoAsync(path: string): Promise<FileInfo> {
	...
	if (!extension) {
		...
		_fileInfo.setDateModified = fileMetaData.modifiedDate;
		_fileInfo.setSize = fileMetaData.size;
		_fileInfo.setMode = fileMetaData.mode;
		return _fileInfo;
	}
...
export async function getShortCutFromURLAsync(path: string): Promise<ShortCut> {
	...
	if (typeof shortCut === 'object') {
		...
		const fileName = (iSCut as { FileName: unknown })?.[
			'FileName'
		] as string;
		const iconPath = (iSCut as { IconPath: unknown })?.[
			'IconPath'
		] as string;
		const fileType = (iSCut as { FileType: unknown })?.[
			'FileType'
		] as string;
