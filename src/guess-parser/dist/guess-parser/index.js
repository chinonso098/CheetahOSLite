			...
			/******/ [
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					exports.findRootModule = function (registry) {
						...
						var roots = allModulePaths.filter(function (m) {
							return !childModules.has(m);
						});
						if (roots.length > 1) {
							throw new Error(
					...
					exports.collectRoutingModules = function (
						rootFile,
						registry,
						result,
						parentFilePath,
						...
						if (parentFilePath === void 0) {
							parentFilePath = rootFile;
						}
						...
						var processRoute = function (r, routePath) {
							...
							r.children.forEach(function (route) {
								return process(route, path);
							});
						...
						declaration.eagerRoutes.forEach(function (r) {
							return processRoute(r);
						});
						declaration.lazyRoutes.forEach(function (r) {
							return processLazyRoute(r);
						});
					};
					exports.findMainModule = function (program) {
						...
						return program.getSourceFiles().reduce(function (
							a,
							sf
						) {
							if (a) {
					...
					var normalizeFilePath = function (path) {
						return path_1.join.apply(
							void 0,
							__spread(
								path.split(/\//).map(function (part, index) {
									return part === '' && index === 0
										? path_1.sep
										: part;
								})
					...
					exports.getModulePathFromRoute = function (
						parentPath,
						loadChildren,
						program,
						host
						...
						var path = path_1.join(
							path_1.dirname(parentPath),
							childModuleFile
								.split('/')
						...
						if (!fs_1.existsSync(path)) {
							throw new Error(
								'The relative path "' +
									...
									'" to "' +
									parentPath +
									'" cannot be resolved to a module'
							);
						}
						return path;
					};
					var imports = function (
						...
						if (
							importCache[parent] &&
							importCache[parent][child] !== undefined
						) {
						...
						sf.forEachChild(function (n) {
							...
							var fullPath = normalizeFilePath(
								resolvedModule.resolvedFileName
							);
							if (fullPath === child) {
								found = true;
					...
					var cache = {};
					exports.cleanModuleCache = function () {
						return (cache = {});
					};
					...
					exports.getModuleEntryPoint = function (
						path,
						entryPoints,
						program,
						host
					) {
						var parents = __spread(entryPoints).filter(
							function (e) {
								return imports(e, path, program, host, cache);
							}
						);
					...
					exports.getLazyEntryPoints = function (
						node,
						program,
						host
					) {
						var value = routes_1.readLoadChildren(
							node,
						...
						var parent = path_1.resolve(
							node.getSourceFile().fileName
						);
						var module = exports.getModulePathFromRoute(
							parent,
							value,
							program,
							host
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					var getObjectProp = function (node, prop) {
						...
						try {
							for (
								...
								var value = val;
								if (
									value.name.kind !== ts.SyntaxKind.Identifier
								) {
									continue;
								}
					...
					exports.getRoute = function (
						...
						var loadChildren = exports.readLoadChildren(
							node,
							program.getTypeChecker()
						);
						if (loadChildren) {
							var parent_2 = modules_1.getModuleEntryPoint(
								path_1.resolve(node.getSourceFile().fileName),
								entryPoints,
								program,
								host
							);
							var module_1 = modules_1.getModulePathFromRoute(
								parent_2,
								loadChildren,
								program,
								host
					...
					exports.isRoute = function (n, typeChecker, redirects) {
						...
						var redirectTo =
							redirects &&
							readRedirect(objLiteral, typeChecker) !== null;
						var children = !!exports.readChildren(objLiteral);
						var loadChildren = !!exports.readLoadChildren(
							objLiteral,
							typeChecker
						);
						var component = !!getObjectProp(
							objLiteral,
							'component'
						);
						return (
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					exports.readFiles = function (dir) {
						...
						var result = fs_1.readdirSync(dir).map(function (node) {
							return path_1.join(dir, node);
						});
						var files = result.filter(function (node) {
							return (
								fs_1.statSync(node).isFile() &&
								(node.endsWith('.jsx') || node.endsWith('.js'))
							);
						});
						var dirs = result.filter(function (node) {
							return fs_1.statSync(node).isDirectory();
						});
						return [].concat.apply(
							files,
							dirs.map(exports.readFiles)
						);
					};
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					var getLazyDefinition = function (
						filename,
						identifier,
						ls
					) {
						var defs = ls.getDefinitionAtPosition(
							filename,
							identifier.pos + 1
						);
						if (!defs) {
					...
					var extractRoute = function (c, file, ls) {
						if (
							c.kind !== ts.SyntaxKind.JsxElement &&
							c.kind !== ts.SyntaxKind.JsxSelfClosingElement
						) {
							return;
						}
					...
					exports.parsePreactJSXRoutes = function (base) {
						...
						var routes = jsxFiles.reduce(function (a, f) {
							return a.concat(
								extractRoutes(
									f,
									language_service_1.getLanguageService(
										rootFileNames,
										options
									)
								)
						...
						return Object.keys(routeMap).map(function (k) {
							return routeMap[k];
						});
					};
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					exports.parseRoutes = function (base) {
						...
						if (!app) {
							throw new Error(
								'Cannot detect the application type'
							);
						}
						if (
							app.type === interfaces_1.ProjectType.AngularCLI &&
							app.details &&
							app.details.tsconfigPath
						) {
							result = angular_1.parseRoutes(
								path_1.join(base, app.details.tsconfigPath)
							);
						}
						if (
							app.type ===
								interfaces_1.ProjectType
									.CreateReactAppTypeScript &&
							app.details &&
							app.details.tsconfigPath
						) {
							result = react_1.parseReactTSXRoutes(
								app.details.tsconfigPath
							);
						}
						if (
							app.type ===
								interfaces_1.ProjectType.CreateReactApp &&
							app.details &&
							app.details.sourceDir
						) {
							result = react_1.parseReactJSXRoutes(
								path_1.join(base, app.details.sourceDir)
							);
						}
						if (
							app.type === interfaces_1.ProjectType.PreactCLI &&
							app.details &&
							app.details.sourceDir
						) {
							result = preact_1.parsePreactJSXRoutes(
								path_1.join(base, app.details.sourceDir)
							);
						}
						if (!result) {
						...
						res.filter(function (r) {
							return !r.parentModulePath || r.path === '/';
						}).forEach(function (r) {
							return (r.parentModulePath = null);
						});
						return res;
					};
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					exports.parseRoutes = function (
						...
						var parsed = ts.parseJsonConfigFileContent(
							config.config,
							parseConfigHost,
							path_1.resolve(path_1.dirname(tsconfig)),
							{
								noEmit: true,
							}
						);
						...
						var visitTopLevelRoutes = function (s, callback, n) {
							...
							if (
								...
								n.forEachChild(
									visitTopLevelRoutes.bind(null, s, callback)
								);
							}
						};
						...
						if (!mainPath) {
							throw new Error(
								'Cannot find the main application module'
							);
						}
						var entryPoints = new Set([mainPath]);
						var collectEntryPoints = function (n) {
							var path = modules_1.getLazyEntryPoints(
								n,
								program,
								host
							);
						...
						program.getSourceFiles().map(function (s) {
							s.forEachChild(
								visitTopLevelRoutes.bind(null, s, function (n) {
									var path = path_1.resolve(
										n.getSourceFile().fileName
									);
									var route = routes_1.getRoute(
										n,
										entryPoints,
										program,
										host
						...
						if (Object.keys(registry).length > 0) {
							modules_1.collectRoutingModules(
								modules_1.findRootModule(registry),
								registry,
								result
							);
						}
				...
				/***/ function (module, exports, __webpack_require__) {
						...
						var _loop_1 = function () {
							...
							if (
								...
								if (el.tagName.text === 'Route') {
									...
									el.attributes.properties.forEach(
										function (p) {
											var text = p.name.text;
											if (text === 'path') {
												module_1.path =
					...
					exports.parseReactRoutes = function (files, options) {
						...
						var routes = jsxFiles.reduce(function (a, f) {
							return a.concat(extractRoutes(f));
						}, []);
						var modules = routes.reduce(function (a, r) {
							a[r.modulePath] = true;
							return a;
						}, {});
						var rootModule = routes
							.filter(function (r) {
								return (
									r.parentModulePath &&
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					var calcProjectFileAndBasePath = function (project) {
						var projectIsDir = fs_1
							.lstatSync(project)
							.isDirectory();
						var projectFile = projectIsDir
							? path_1.join(project, 'tsconfig.json')
							: project;
						var projectDir = projectIsDir
							? project
							: path_1.dirname(project);
						var basePath = path_1.resolve(
							process.cwd(),
							projectDir
						);
					...
					exports.parseRoutes = function (tsconfig) {
						var _a = ts.readConfigFile(tsconfig, function (f) {
								return fs_1.readFileSync(f).toString();
							}),
							config = _a.config,
							error = _a.error;
						if (error) {
						...
						var basePath =
							calcProjectFileAndBasePath(tsconfig).basePath;
						var parsed = ts.parseJsonConfigFileContent(
							config,
							parseConfigHost,
							basePath
						);
						return _1.parseReactRoutes(
							parsed.fileNames,
							parsed.options
						);
					};
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					var fs_1 = __webpack_require__(2);
					var ts = __webpack_require__(1);
					exports.getLanguageService = function (
						rootFileNames,
						options
					) {
						var files = {};
						...
						var servicesHost = {
							...
							getScriptVersion: function (fileName) {
								return (
									files[fileName] &&
									files[fileName].version.toString()
								);
							},
							...
							getDefaultLibFileName: function (o) {
				...
				/***/ function (module, exports, __webpack_require__) {
					...
					var devDep = function (p) {
						return function (name) {
							return p.devDependencies
								? p.devDependencies[name]
								: undefined;
						};
					};
					exports.detect = function (base) {
						var path = ['package.json', '../package.json']
							.map(function (p) {
								return path_1.join(base, p);
							})
							.filter(fs_1.existsSync)
							.pop();
						if (!path) {
							throw new Error(
								'Unable to discover the project type'
							);
						}
						var content = JSON.parse(
							fs_1.readFileSync(path).toString()
						);
						var exists = function (file) {
							return fs_1.existsSync(path_1.join(base, file));
						};
						var d = dep(content);
						...
						if (
							...
							return {
								type: interfaces_1.ProjectType
									.CreateReactAppTypeScript,
								version: d('react-scripts-ts'),
								details: {
									typescript: dd('typescript'),
									tsconfigPath: path_1.join(
										base,
										'tsconfig.json'
									),
									sourceDir: 'src',
